...
impl<Model> ManyResponse<Model> {
  ...
  pub fn new(data: Vec<Model>, total: usize, page: usize, page_size: Option<usize>) -> ManyResponse<Model> {
    let count = data.len();
    let page_count = page_size.map(|page_size| total / page_size + if total % page_size != 0 { 1 } else { 0 });
